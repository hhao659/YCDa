...
def swap_channels_and_measure(
    ...
    target_shape = (
        min(salient_img.shape[0], camouflaged_img.shape[0]),
        min(salient_img.shape[1], camouflaged_img.shape[1]),
        3,
    )
    ...
    print(
        f"SSIM (Camouflaged vs. Salient): Before swap = {ssim_before_camouflaged:.4f}, After swap = {ssim_after_camouflaged:.4f}"
    )
...
def main():
    ...
    parser.add_argument(
        "--function", choices=["decouple", "swap"], required=True, help="Function to run: decouple or swap"
    )
    ...
    parser.add_argument(
        "--output_swap_camouflaged", default="swapped_camouflaged.png", help="Output path for swapped camouflaged image"
    )
    parser.add_argument(
        "--output_swap_salient", default="swapped_salient.png", help="Output path for swapped salient image"
    )
    ...
    if args.function == "decouple":
        ...
        swap_channels_and_measure(
            args.salient, args.camouflaged, args.gt_mask, args.output_swap_camouflaged, args.output_swap_salient
        )
